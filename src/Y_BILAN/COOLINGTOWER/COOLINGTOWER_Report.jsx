...
const computeOpexCosts = (innerData) => {
  ...
  return { elecRows, totalElec, coutElec, co2Elec, conso_air, coutAir, co2Air, eauRows, coutEau, reactifRows, coutReactifs, co2TransportReactifs, energieRows, coutEnergie, co2Energie, coutTransportResidus, co2TransportResidus, coutTransportReactifs, totalCout_h, totalCout_an, totalCO2_kgh, currency, availability };
};

const OpexSummary = ({ opex, t }) => {
  const { elecRows, totalElec, coutElec, co2Elec, coutAir, co2Air, eauRows, coutEau, reactifRows, coutReactifs, co2TransportReactifs, energieRows, coutEnergie, co2Energie, coutTransportResidus, co2TransportResidus, coutTransportReactifs, totalCout_h, totalCout_an, totalCO2_kgh, currency, availability } = opex;
  if (totalElec === 0 && coutEnergie === 0 && coutEau === 0) return <p style={{ color: '#999', fontSize: 12, padding: '10px 14px' }}>{t('Coûts OPEX non disponibles — ouvrir les onglets Design et Opex.')}</p>;
  return (
    <div>
    ...
    <div style={styles.container}>
      ...
      <Section title={t('2. Émissions polluantes')}>
        ...
        <SubSection title={t('Résidus calculés')}>
          ...
          <KV label={t('Cendres lourdes humides [kg/h]')} value={fmt(Residus.WetBottomAsh)} />
        ...
        <div style={styles.twoCol}>
          ...
          <div>
            ...
            <SubSection title={t('Énergie auxiliaire')}>
              <KV label={t('Gaz haute valeur [MW]')} value={fmt(innerData.conso_gaz_H)} />
              <KV label={t('Fuel [MW]')} value={fmt(innerData.conso_fuel)} />
