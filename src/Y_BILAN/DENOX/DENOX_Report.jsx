...
const DENOX_Report = ({ innerData = {}, currentLanguage = 'fr' }) => {
  ...
  const FG_OUT_mass = innerData.FG_DENOX_out_reheating || innerData.FG_OUT_mass || {};
  ...
  const FG_OUT_vol = innerData.FG_DENOX_OUT || innerData.FG_OUT_vol || _nm3Computed;
  ...
  const reactifConsumption = [
    { label: t('NH₃ (Ammonia) [kg/h]'), value: innerData.Conso_Ammonia },
    { label: t('NaOH [kg/h]'), value: innerData.Conso_NaOH },
    { label: t('CAP [kg/h]'), value: innerData.Conso_CAP },
  ].filter(r => parseFloat(r.value) > 0);
